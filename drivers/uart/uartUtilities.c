...
void uart_us_sendbuffer(u8 SendBuffer[N_UARTS][TEST_BUFFER_SIZE], u32 *us_distance, const char * text, int DATACOUNT[N_UARTS]{	
	char c;	
    for(text; c = *text; text++) {
        SendBuffer[0][DATACOUNT[0]] = c;
        DATACOUNT[0]++;
    }
    SendBuffer[0][DATACOUNT[0]]   = us_distance/100 % 10 + '0';
    SendBuffer[0][DATACOUNT[0]+1] = us_distance/10 % 10 + '0';
    SendBuffer[0][DATACOUNT[0]+2] = us_distance % 10 + '0';
    DATACOUNT[0] = DATACOUNT[0]+3;
}
...
/**
 * @brief Function to set the timeout function for an xuartlite.
 * 
 * @param InstancePtr 
...
{
    InstancePtr->TimeOutTimer = FunctPtr;
}